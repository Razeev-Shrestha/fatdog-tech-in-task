...
import { addUsersToCity, createUser, deleteUser, deleteUserFromCity, getUserByName, getUsers } from '@/services'
...
export const getUsersController = async () => {
	try {
		...
		return NextResponse.json({ payload: users, message: 'Users fetched successfully.', success: true }, { status: 200 })
	} catch (err) {
		console.error(err)
...
export const createUserController = async (req: NextRequest) => {
	try {
		...
		if (result.length > 0) {
			return NextResponse.json({ payload: null, message: 'User already exists.', success: false }, { status: 400 })
		}

		const user = await createUser(body)

		return NextResponse.json({ payload: user, message: 'User created successfully.', success: true }, { status: 201 })
	} catch (err) {
		console.error(err)
		return NextResponse.json({ payload: null, message: 'Internal server error.', success: false }, { status: 500 })
	}
...
export const deleteUserController = async (req: NextRequest) => {
	try {
		...
		return NextResponse.json({ payload: user, message: 'User deleted successfully.', success: true }, { status: 200 })
	} catch (err) {
		console.error(err)
		return NextResponse.json({ payload: null, message: 'Internal server error.', success: false }, { status: 500 })
	}
